...
for courseId in resultsCertRepeatedCourse.keys():
	...
	if len(someResultsCertRepeatedCourse) > 0:
		handle, = plt.plot(np.arange(len(someResultsCertRepeatedCourse)) + 1, someResultsCertRepeatedCourse*100., 'c-.')
		handles.append(handle)
		plt.plot(np.arange(len(someResultsCertRepeatedCourse)) + 1, someResultsCertRepeatedCourse*100., 'co')
	...
	if len(someResultsCert) > 0:
		...
		names.append("Certify")
	plt.legend(handles, names, loc="lower right")
	plt.title(courseId)
	plt.xlabel("Week #")
	plt.ylabel("Accuracy (%)")
	plt.xlim((0., 7.))
